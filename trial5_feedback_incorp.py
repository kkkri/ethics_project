...
def initialize_graph(n):
    ...
    G = nx.Graph()
    for i in range(n):
        G.add_node(i, attendance=True, false_votes=0, detected_as_false_present=False)
...
def perform_roll_call(G, confirmations, m):
    ...
    return false_positive, penalties, false_voters_count
...
def initialize_colluding_groups(G, num_groups=5):
    ...
    group_size = len(colluders) // num_groups
    colluding_groups = [colluders[i:i + group_size] for i in range(0, len(colluders), group_size)]
    
    # Each round, rotate attendance for colluding groups
    return colluding_groups

def rotate_group_attendance(G, colluding_groups, round_num):
    """Update attendance for colluding groups by rotating which group is absent."""
    for group in colluding_groups:
        for i, student in enumerate(group):
            # Every group rotates who will be absent and who will mark false presents
            G.nodes[student]['attendance'] = (i == (round_num % len(group)))
    ...
    for k in k_values:
        for m in m_values:
            ...
            for round_num in range(simulation_rounds):
                ...
                false_positive, penalties, false_voters_count = perform_roll_call(G, confirmations, m)
                total_false_positives += false_positive
                total_penalties += sum(penalties.values())
                total_false_voters += false_voters_count
            ...
            results = results._append({'n': n, 'k': k, 'm': m, 
                                       ...
                                       'false_voters_avg': avg_false_voters}, ignore_index=True)
