...
# Run the simulations with structured collusion behavior
# for n in n_values:
#     for k in k_values:
#         for m in m_values:
...
#                 total_false_voters += sum([G.nodes[peer]['false_votes'] for peer in G.nodes])
...
#                                        'false_voters_caught_avg': avg_false_voters_caught}, ignore_index=True)
...
for n in n_values:
    for k in k_values:
        for m in m_values:
            total_false_positives = 0
            total_penalties = 0
            total_false_voters = 0  # Total unique false voters across all rounds
            total_false_presents_detected = 0
            total_false_voters_caught = 0
            ...
            for round_num in range(simulation_rounds):
                ...
                total_false_positives += false_positive
                total_penalties += sum(penalties.values())
                # Track all students who gave false confirmations in this round
