...
def initialize_graph(n):
    """Initialize the graph with students, set up collusion, and assign trust scores and penalty counters."""
    G = nx.Graph()
    for i in range(n):
        G.add_node(i, attendance=True, false_votes=0, false_confirmations=0, detected_as_false_present=False,
                   trust_score=1.0, penalty_level=0, rounds_without_penalty=0)  # penalty_level and rounds_without_penalty for enhanced penalties and rehab
...
def escalate_penalties(G, penalties):
    """Apply escalating penalties based on penalty levels."""
    for student, penalty_count in penalties.items():
        # Increase penalty level for repeat offenders, increasing penalty by the level factor
        penalty_level = G.nodes[student]['penalty_level']
        new_penalty = penalty_count * (1 + penalty_level * 0.5)  # Escalation factor by 50% per level
        G.nodes[student]['penalty_level'] += 1  # Increase penalty level on each offense
        G.nodes[student]['trust_score'] = max(0.1, G.nodes[student]['trust_score'] - new_penalty * 0.1)
        G.nodes[student]['rounds_without_penalty'] = 0  # Reset rehabilitation count

def rehabilitate_students(G):
    """Rehabilitate students by reducing penalties and improving trust for those without recent penalties."""
    for student in G.nodes:
        # Increment rounds without penalty, and if they reach a threshold, reduce penalty level and improve trust score
        if G.nodes[student]['rounds_without_penalty'] > 5 and G.nodes[student]['penalty_level'] > 0:
            G.nodes[student]['penalty_level'] -= 1  # Reduce penalty level
            G.nodes[student]['trust_score'] = min(1.0, G.nodes[student]['trust_score'] + 0.1)  # Gradual recovery of trust
        else:
            G.nodes[student]['rounds_without_penalty'] += 1  # Increment rounds without penalty
...
# Simulation loop with enhanced penalty and rehabilitation system
for n in n_values:
    for k in k_values:
        for m in m_values:
            ...
            for round_num in range(simulation_rounds):
                ...
                # Generate colluding groups and confirmations (similar to previous code)
                ...
                # Apply escalating penalties and update trust scores
                escalate_penalties(G, penalties)
                
                # Increment rounds without penalties for each student, enabling rehabilitation
                rehabilitate_students(G)
